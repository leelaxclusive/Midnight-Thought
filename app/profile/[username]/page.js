...
import { BookOpen, Heart, Eye, Users, Calendar, MapPin, Link as LinkIcon, MessageCircle, Clock, TrendingUp, Award, UserPlus, UserMinus } from "lucide-react";
			...
			<div className="min-h-screen bg-background">
				...
				<div className="max-w-6xl mx-auto px-4 py-8">
					<div className="text-center py-12">
						...
						<p className="text-foreground mb-4">The author you&apos;re looking for doesn&apos;t exist.</p>
						<Button asChild>
	...
	return (
		<div className="min-h-screen bg-background">
			...
			<div className="relative">
				{/* Cover Image */}
				<div className="h-60 bg-gradient-to-r from-primary/20 via-primary/10 to-primary/20 relative">
					{author.cover && <Image src={author.cover} alt="Profile cover" fill className="object-cover" priority />}
							...
							<div className="flex-1 min-w-0">
								<div className="flex flex-col sm:flex-row sm:items-center sm:justify-between">
									<div>
										...
										<p className="text-lg text-foreground">@{author.username}</p>
									...
									{session && !isOwnProfile && (
										<Button onClick={handleFollow} variant={isFollowing ? "secondary" : "default"} className="mt-4 sm:mt-0">
											{isFollowing ? (
												<>
								...
								<div className="flex flex-wrap gap-6 mt-4 text-sm">
									...
									<div className="flex items-center space-x-1">
										...
										<span className="text-foreground">total likes</span>
									</div>
								</div>
							</div>
						</div>
						...
						<Card>
							...
							<CardContent className="space-y-3">
								<div className="flex justify-between">
									<span className="text-sm text-foreground">Monthly Views</span>
									<span className="font-medium">{stats.monthlyViews?.toLocaleString()}</span>
								</div>
								<div className="flex justify-between">
									<span className="text-sm text-foreground">Monthly Likes</span>
									<span className="font-medium">{stats.monthlyLikes}</span>
								</div>
								<div className="flex justify-between">
									<span className="text-sm text-foreground">Total Words</span>
								...
								<div className="flex justify-between">
									...
									<span className="font-medium">
										{Math.floor(stats.readingTime / 60)}h {stats.readingTime % 60}m
									</span>
								</div>
							</CardContent>
						</Card>
							...
							<TabsContent value="stories" className="space-y-6">
										...
										<Card key={story._id || story.slug} className="hover:shadow-lg transition-shadow">
											<CardHeader>
												<div className="flex justify-between items-start mb-2">
													...
													<div className="text-sm text-foreground">Updated {getTimeAgo(story.updatedAt)}</div>
												</div>
											...
											<CardContent>
												<p className="text-foreground mb-4 line-clamp-3 leading-relaxed">{story.description}</p>
												...
												<div className="flex justify-between items-center text-sm text-foreground">
													...
													<Button asChild size="sm">
														<Link href={`/story/${story.slug}`}>Read Story</Link>
													</Button>
												</div>
											</CardContent>
										</Card>
								...
								{stories.length === 0 && (
									<div className="text-center py-12">
										...
										<p className="text-foreground">{isOwnProfile ? "Start writing your first story!" : `${author.name} hasn't published any stories yet.`}</p>
									</div>
								)}
							</TabsContent>
							...
							<TabsContent value="activity" className="space-y-6">
								...
								<div className="space-y-4">
									<div className="text-center py-12">
										...
										<p className="text-foreground">Activity will appear here once the author starts interacting.</p>
									</div>
								</div>
							</TabsContent>
										...
										<div className="prose prose-sm max-w-none">
											...
											<div className="mt-6 grid grid-cols-2 gap-4 text-sm">
												<div>
													...
													<p className="text-foreground">{author.joinedAt.toLocaleDateString()}</p>
												</div>
												<div>
													<h4 className="font-medium mb-2">Favorite Genre</h4>
													<p className="text-foreground">{stats.mostPopularGenre}</p>
												</div>
												<div>
													<h4 className="font-medium mb-2">Stories Published</h4>
													<p className="text-foreground">{author.totalStories}</p>
												</div>
												<div>
													<h4 className="font-medium mb-2">Total Words Written</h4>
													<p className="text-foreground">{stats.totalWords?.toLocaleString()}</p>
												</div>
											</div>
										</div>
									</CardContent>
