      ...
      <Card>
        <CardContent className="p-4">
          <div className="flex items-start justify-between mb-4">
            ...
            <Badge variant={currentChapter.status === 'published' ? 'default' : 'secondary'}>
              ...
              <div className="grid grid-cols-2 md:grid-cols-3 gap-4 text-sm">
                <div className="text-center">
                  <div className="font-medium text-foreground">
                    {Math.round(currentChapter.analytics.completionRate || 0)}%
                  </div>
                  <div className="text-xs text-muted-foreground">Completion Rate</div>
                </div>
                <div className="text-center">
                  <div className="font-medium text-foreground">
                    {Math.round((currentChapter.analytics.averageReadTime || 0) / 60)}m
                  </div>
                  <div className="text-xs text-muted-foreground">Avg. Read Time</div>
                </div>
                <div className="text-center">
                  <div className="font-medium text-foreground">
                    {currentChapter.analytics.uniqueReaders || 0}
                  </div>
        ...
        <div className="max-h-96 overflow-y-auto">
          {chapters.map((chapter) => (
            <Link
              ...
              className={cn(
                'block p-4 border-b border-border last:border-b-0 hover:bg-muted/50 transition-colors',
                currentChapter._id === chapter._id && 'bg-primary/10 border-l-4 border-l-primary'
              )}
            >
